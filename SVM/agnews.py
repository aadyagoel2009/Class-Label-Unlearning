...
def prepare_ag_news(limit_train: int = None, limit_test: int = None, seed: int = 42):
    """
    AG News (train/test provided). Labels are 0..3.
    Optionally subsample for faster runs.
    """
    ds = load_dataset("ag_news")

    # Some versions have 'text', others 'title'+'description'
    def join_fields(r):
        if "text" in r:
            return r["text"]
        return f"{r.get('title','')} {r.get('description','')}".strip()

    train_texts = [join_fields(r) for r in ds["train"]]
    test_texts  = [join_fields(r) for r in ds["test"]]
    ...
    if limit_train is not None:
        ...
        idx = rng.choice(len(train_texts), size=limit_train, replace=False)
        train_texts = [train_texts[i] for i in idx]
        y_train = y_train[idx]
    if limit_test is not None:
        rng = np.random.default_rng(seed + 1)
        idx = rng.choice(len(test_texts), size=limit_test, replace=False)
...
def main():
    train_docs, test_docs, y_train, y_test = prepare_ag_news()
