...
def evaluate_accuracy(theta, vectorizer, docs: list, labels: np.ndarray):
    """
    Compute classification accuracy; optionally mask one class.
    ...
    Inputs:
      ...
      - mask_class: class index to mask (or None)
...
def compute_mia_auc(attack_clf, theta, vectorizer, train_docs: list, 
                    train_labels: np.ndarray, test_docs: list, test_labels: np.ndarray):
    """
...
def compute_class_mia_auc(attack_clf, theta, vectorizer, train_docs, train_labels, test_docs,  test_labels, cls: int):
    ...
    """
    # select only examples of class cls
    ...
    # delegate to existing compute_mia_auc
...
def main():
    ...
    # remove all samples of mask_cls
    ...
    # fine-tune on reduced set, preserving original class slots
    ...
    theta_init = theta_un[classes_red, :] # init for present classes

    ft = LogisticRegression(penalty='l2', C=C, solver='lbfgs', fit_intercept=False, warm_start=True, max_iter=5, random_state=42)
    ...
    print(f"Class label unlearned: {int(mask_cls):.4f}")
    ...
    print(f"MIA AUC after unlearning: {auc_after:.4f}")
