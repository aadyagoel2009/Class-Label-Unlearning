...
def prepare_bbc_news(test_size: float = 0.2, random_state: int = 42):
    from datasets import load_dataset
    import numpy as np
    from sklearn.model_selection import train_test_split

    ds = load_dataset("SetFit/bbc-news")  # features: 'text', 'label'
    split_name = "train" if "train" in ds else list(ds.keys())[0]

    # Convert to Python list to avoid HF column indexing issues
    all_texts  = list(ds[split_name]["text"])
    all_labels = np.array(ds[split_name]["label"], dtype=int)
...
def main():
    train_docs, test_docs, y_train, y_test = prepare_bbc_news()
